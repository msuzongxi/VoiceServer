...
def ibcf_predict(x, user_id, item_id, threshold):
    target = x[item_id, ]
    others = np.delete(x, [item_id], axis=0)
    others = others[~np.isnan(others[:, user_id])]
    sim = np.array([pearson(target, item) for item in others])
    weights = sim[sim>threshold]
    ratings = others[sim>threshold, user_id]
    if weights.size == 0:
        return -1
...
if __name__== "__main__":
    ...
    tbp_songs = np.setdiff1d(_200_songs, subject_ratings.keys()) #excluding rated songs
    predicted_ratings = []
    for song_id in tbp_songs:
        int_song_id = int(song_id)
        song_rating = ibcf_predict(ui_matrix, 0, int_song_id, 0.2)
